...
'''
Modifications by https://github.com/AurelianTactics
To make it more gym friendly
'''


class ATDiceAdventure:
    def __init__(self,
                 level=1,
                 limit_levels=None,
        ...
        '''
        is_auto_reset: bool
            If True, when a level is over, resets the level before the state can
            be updated and sent to the env (default for dice_adventure)
        '''

        # added by AT
        self.is_auto_reset = is_auto_reset
        self.is_tower_reached = False
        self.is_truncated = False
        self.at_game_timesteps = 0
        self.at_max_game_timesteps = 10000
    ...
    def next_level(self):
        ...
        :return:
        """
        # goal is reached or the players are dead. not sure why not done in those spots
        # want to keep my changes to minimum so updating here
        ...
        if self.is_auto_reset:
            self.reset_board_and_level()

    def reset_board_and_level(self):
        '''
        Added to allow game to send a state back with done conditions being met
        rather than auto-going to the next level
        '''
        self.terminated = False
        self.is_tower_reached = False
        self.is_truncated = False
    ...
    def execute_action(self, player, action):
        ...
        # if self.render_game:
        #    self.render()
        # added by AT for possibly truncating
        self.increment_timestep_update_truncated()
    ...
    # Added by AT #
    ###########
    def increment_timestep_update_truncated(self):
        self.at_game_timesteps += 1
        if self.at_game_timesteps >= self.at_max_game_timesteps:
